...
class TlEditCourseScreen extends ConsumerStatefulWidget {
  final String courseId;
  const TlEditCourseScreen({super.key, required this.courseId});

  @override
  ConsumerState<TlEditCourseScreen> createState() => _TlEditCourseScreenState();
}

class _TlEditCourseScreenState extends ConsumerState<TlEditCourseScreen> {
  ...
  void _loadCourse() {
    final course = ref.read(courseProvider).list.firstWhere(
      (c) => c.id == widget.courseId,
      orElse: () => throw Exception('Course not found'),
    );

    _c['name'] = TextEditingController(text: course.courseName);
    _c['desc'] = TextEditingController(text: course.description);
    _c['duration'] = TextEditingController(text: course.duration);
    _c['fees'] = TextEditingController(text: course.totalFees.toString());
    _c['online'] =
        TextEditingController(text: course.totalSeatsOnline.toString());
    _c['offline'] =
        TextEditingController(text: course.totalSeatsOffline.toString());
    _c['tirunelveli'] =
        TextEditingController(text: course.totalSeatsTirunelveli.toString());
    _c['tisaiyanvilai'] =
        TextEditingController(text: course.totalSeatsTisaiyanvilai.toString());
    // ? FIX: category is no longer editable here � see note near
    // the dropdown removal below.

    _start = DateTime.tryParse(course.startDate);
    _end = DateTime.tryParse(course.endDate);
    if (course.registrationStartDate.isNotEmpty) {
      _regStart = DateTime.tryParse(course.registrationStartDate);
    }
    if (course.registrationEndDate.isNotEmpty) {
      _regEnd = DateTime.tryParse(course.registrationEndDate);
    }
  ...
  Future<void> _submit() async {
    ...
    final data = {
      ...
      'totalSeatsTirunelveli': int.tryParse(_c['tirunelveli']?.text ?? '0') ?? 0,
      'totalSeatsTisaiyanvilai': int.tryParse(_c['tisaiyanvilai']?.text ?? '0') ?? 0,
      ...
      // Category is fixed at creation time.
    };
    ...
    if (ok) {
      _snack('Course updated!', AppColors.success);
      Navigator.pop(context);
    } else {
      _snack(ref.read(courseProvider).error ?? 'Update failed',
          AppColors.error);
    }
  }
          ...
          child: ResponsiveCentered(
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                children: [
                  _sec('Course Info'),
                  _f('Course Name', 'name', Icons.menu_book_rounded, req: true),
  ...
  Widget _datePicker(String label, DateTime? date, bool isStart, {bool isReg = false}) => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(label,
              style: TextStyle(
