...
class TlApplyLeaveScreen extends ConsumerStatefulWidget {
  const TlApplyLeaveScreen({super.key});
  @override
  ConsumerState<TlApplyLeaveScreen> createState() =>
      _TlApplyLeaveScreenState();
}

class _TlApplyLeaveScreenState extends ConsumerState<TlApplyLeaveScreen> {
  ...
  Future<void> _fetchHistory() async {
    ...
    final res = await LeaveService.getMyLeaveHistory();
    if (!mounted) return;
    if (res['success'] == true) {
      final d = res['data'];
      if (d is List) {
  ...
  Future<void> _submit() async {
    ...
    final data = {
      ...
      'leaveType': _isHalfDay ? 'HALF_DAY' : 'FULL_DAY',
    };

    final success =
        await ref.read(leaveProvider.notifier).applyOwnLeave(data);
    if (!mounted) return;
            ...
            child: Row(children: [
              ...
              const Column(
                  ...
                  children: [
                    ...
                    Text('Submit leave request to Admin',
              ...
              child: Column(children: [
                ...
                Container(
                  ...
                  child: const Row(children: [
                    ...
                    Expanded(
                      child: Text(
                        'Leave request will be sent to Admin for approval. '
    ...
    if (_loadingHistory) {
      ...
      content = Column(
        children: _history.map((item) {
          final type = item['leaveType']?.toString() ?? 'Leave';
          final startDate = item['startDate']?.toString() ?? '';
          final endDate = item['endDate']?.toString() ?? '';
          final date = startDate == endDate ? startDate : '$startDate to $endDate';
