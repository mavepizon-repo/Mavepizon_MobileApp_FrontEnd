...
class TlCreateInternshipScreen extends ConsumerStatefulWidget {
  const TlCreateInternshipScreen({super.key});
  @override
  ConsumerState<TlCreateInternshipScreen> createState() =>
      _TlCreateInternshipScreenState();
}

class _TlCreateInternshipScreenState extends ConsumerState<TlCreateInternshipScreen> {
  ...
  Future<void> _pickReg(bool isStart) async {
    final d = await showDatePicker(
        context: context,
        initialDate: DateTime.now(),
        firstDate: DateTime(2024),
  ...
  Future<void> _submit() async {
    ...
    if (_start == null || _end == null || _regStart == null || _regEnd == null) {
      _snack(
          'Select start, end, registration start and registration end dates',
          AppColors.error);
      return;
    }
    final data = {
      ...
      'registrationStartDate': _dateOnly(_regStart!),
      'registrationEndDate': _dateOnly(_regEnd!),
      'category': 'INTERNSHIP',
    };
    final ok = await ref.read(internshipProvider.notifier).create(data);
    if (!mounted) return;
    if (ok) {
      _snack('Internship created!', AppColors.success);
      Navigator.pop(context);
    } else {
      _snack(ref.read(internshipProvider).error ?? 'Failed',
          AppColors.error);
    }
  }
          ...
          child: Row(children: [
            ...
            const Column(
                ...
                children: [
                  ...
                  Text('Add a new internship program',
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                  ...
                  children: [
                    ...
                    _f('Internship Name', 'name', Icons.work_rounded,
                        req: true),
                    ...
                    _sec('Schedule'),
                    Row(children: [
                      Expanded(child: _dp('Start Date', _start, true)),
                      const SizedBox(width: 12),
                      Expanded(child: _dp('End Date', _end, false)),
                    ]),
                    Row(children: [
                      Expanded(
                          child: _dpr('Reg. Start Date', _regStart, true)),
                      const SizedBox(width: 12),
                      Expanded(
                          child: _dpr('Reg. End Date', _regEnd, false)),
                    ]),
  ...
  Widget _f(String label, String key, IconData icon,
      {String? hint, TextInputType? type, int lines = 1, bool req = false}) {
  ...
  Widget _dp(String label, DateTime? date, bool isStart) =>
  ...
  Widget _dpr(String label, DateTime? date, bool isStart) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label,
            style: TextStyle(
                fontSize: 13,
